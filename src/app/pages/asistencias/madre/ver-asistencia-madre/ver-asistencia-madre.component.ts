...
@Component({
  selector: 'app-ver-asistencia-madre',
  templateUrl: './ver-asistencia-madre.component.html',
  styleUrls: ['./ver-asistencia-madre.component.css']
})
export class VerAsistenciaMadreComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.usuarioService.madrePorPersona().subscribe(({ ok, madre }) => {
      if (ok) {
        this.madre = madre;
        this.alumnoService.listaAlumnosPorMadre(Number(madre.id)).subscribe({
          next: ({ok, alumnos})=>{
            if(ok){
